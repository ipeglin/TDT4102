...
int main() {
    // Oppgave 1
    std::unique_ptr<Car> car_1 (new Car(5));
    std::unique_ptr<Car> car_2 (new Car(0));
    std::unique_ptr<Car> car_3 (new Car(4));

    std::shared_ptr<Person> person_1(new Person("Ola Normann", "ola@normann.no"));
    std::shared_ptr<Person> person_2(new Person("Kari Normann", "kari@normann.no", std::move(car_1)));
    std::shared_ptr<Person> person_3(new Person("Leif Eriksson", "leif.eriksson@viking.no", std::move(car_3)));
    std::shared_ptr<Person> person_4(new Person("Olav Haraldsson", "olavtheholy@viking.no", std::move(car_2)));
    ...
    // // Oppgave 3
    Meeting meeting_1(11, 12, 14, Campus::Trondheim, "How to pass TDT4102 without a cookbook 101", person_1);
